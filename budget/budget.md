...
이번 미션은 특정 프로그래밍 언어로 파일 입출력 기반의 가계부 콘솔 프로그램을 구현하는 과제입니다. 수입과 지출 내역을 단순히 저장하는 수준을 넘어, 수정/삭제, 검색, 월별 요약, 카테고리 관리, 예산 초과 경고까지 포함한 "작은 서비스" 형태로 완성합니다.
...
```text
<실행 명령> budget_app <command> [options]
```
...
선택한 프로그래밍 언어 및 실행 환경의 지원 버전을 사용한다.
...
```text
$ <실행 명령> budget_app category add
카테고리명: food
[저장 완료] category=food

$ <실행 명령> budget_app category list
- food
- transport
```
...
```text
$ <실행 명령> budget_app budget set --month 2024-01 --amount 500000
[저장 완료] 2024-01 예산 500000원

$ <실행 명령> budget_app summary --month 2024-01 --top 3
총 수입: 3000000원
총 지출: 215000원
잔액: 2785000원
예산: 500000원 (사용률 43.0%)
...
```text
$ <실행 명령> budget_app export --out export.csv --month 2024-01
[완료] export.csv (12 records)

$ <실행 명령> budget_app import --from import.csv
